    ...
    return (
      <div className="card">
        <div className="card-body text-center py-12">
          <p className="text-neutral-600">{t({ en: 'No complaints found', ur: 'کوئی شکایت نہیں ملی' }, language)}</p>
        </div>
      </div>
    );
  }
            ...
            {complaints.map(complaint => (
              <tr key={complaint.id} className="hover:bg-neutral-50">
                <td className="px-6 py-4">
                  <div>
                    <p className="font-medium text-neutral-900 truncate max-w-xs">{complaint.title}</p>
                    <p className="text-xs text-neutral-500">ID: {complaint.id}</p>
                  </div>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <select
                    ...
                    className="text-sm border border-neutral-300 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-secondary-500"

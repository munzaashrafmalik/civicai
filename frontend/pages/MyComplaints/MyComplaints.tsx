        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isLoading ? (
            ...
            <div className="card text-center py-16">
              <svg className="mx-auto w-16 h-16 text-secondary-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {filter !== 'all' && (
                <button onClick={() => setFilter('all')} className="text-primary-600 hover:underline text-sm">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6" role="list" aria-label={t({ en: 'Complaints list', ur: 'شکایات کی فہرست' }, language)}>
              {filteredComplaints.map(complaint => (
                <IssueCard key={complaint._id || complaint.complaintId} complaint={{ ...complaint, id: complaint.complaintId || complaint._id || '' }} language={language} />
              ))}
            </div>
          )}
